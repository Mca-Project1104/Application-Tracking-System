...
//   const { currency, candidate, applications } = useAppContext();
...
//         { jobId: id, candidateId: candidate._id },
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
//           },
...
//             <div
//               className={`grid  gap-4 ${userRole === "company" ? "lg:grid-cols-1" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-5"}`}
//             >
...
//               {/* ✅ FIX 1: Changed !userRole === "company" → userRole !== "company" */}
//               {userRole !== "company" && (
//                 <div>
//                   <label
//                     htmlFor="location"
...
//                               Apply Now
//                             </button>
//                             <button>{applications[0].status}</button>
//                           </div>
//                         )}
//                       </div>
//                     </div>
...
const JobListings = () => {
  ...
  const userRole = localStorage.getItem("userRole");
  const token = localStorage.getItem("token");
  const { currency, candidate, application } = useAppContext();
  ...
  // ✅ Correctly extract applied job IDs from context data
  useEffect(() => {
    if (application) {
      const jobIds = application
        .map((app) => {
          // jobId can be object with _id or just string
  ...
  const getApplicationStatus = (jobId) => {
    if (!application?.data?.data) return null;
    const app = application.data.data.find(
              ...
              currentJobs.map((job, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* ✅ Applied badge bar */}
                    {isApplied && (
                      <div className="bg-green-500 dark:bg-green-600 h-1"></div>
                    )}
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2 flex-wrap">
                              ...
                              {/* ✅ Applied label with status */}
                              {isApplied && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 capitalize">
                                  ✓ {appStatus || "Applied"}
                                </span>
                              )}
                            </div>
                      ...
                      {isApplied && application?.data?.data && (
